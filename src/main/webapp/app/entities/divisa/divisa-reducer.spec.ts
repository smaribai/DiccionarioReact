...
import reducer, { createEntity, deleteEntity, getEntities, getEntity, updateEntity, partialUpdateEntity, reset } from './divisa.reducer';
...
describe('Entities reducer tests', () => {
  ...
  describe('Actions', () => {
    ...
    it('dispatches FETCH_DIVISA_LIST actions', async () => {
    ...
    it('dispatches FETCH_DIVISA actions', async () => {
      const expectedActions = [
        {
    ...
    it('dispatches CREATE_DIVISA actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(createEntity({ codigoDivisa: 'CBA' }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches UPDATE_DIVISA actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(updateEntity({ codigoDivisa: 'CBA' }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches PARTIAL_UPDATE_DIVISA actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(partialUpdateEntity({ codigoDivisa: 'ABC' }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches DELETE_DIVISA actions', async () => {
      const expectedActions = [
        {
